...
public class GameFieldView extends View implements Runnable, View.OnTouchListener, Serializable{
    ...
    private transient GameFieldView _this;
    private ArrayList<City> _cities;
    private ArrayList<Silo> _silos;
    private transient ArrayList<Renderable> _renderList;
    ...
    private transient PositionedPath _terrainPath;
    ...
    private boolean _haveRestoreStateRequest = false;
    private GameFieldView _oldState;
    ...
    private transient SoundPool _soundPool;
    private transient Timer _animationTimer;

    // Preferences
    private int _initialScudSpeed;
    private int _scudsPerLevel;
    ...
    public void updatePreferences(int initialScudSpeed, int scudsPerLevel){
        _initialScudSpeed = initialScudSpeed;
        _scudsPerLevel = scudsPerLevel;
        startNewGame();
    }
    ...
    private void updateGameItems(){
        ...
        if(_haveRestoreStateRequest){
            int i = 0;
            GameFieldView oldState = _oldState;
            _state = GameState.paused;

            _currentLevel = oldState._currentLevel;
            for(City thisCity : _cities){
                thisCity._state = oldState._cities.get(i++)._state;
            }
            i = 0;
            for(Silo thisSilo: _silos){
                thisSilo._state = oldState._silos.get(i)._state;
                thisSilo.getMissile().restoreState(oldState._silos.get(i).getMissile());
                ++i;
            }
            i = 0;

            for(Missile thisMissile: _enemyMissiles){
                thisMissile.restoreState(oldState._enemyMissiles.get(i++));
            }
            i = 0;
            for(GameLevel thisLevel : _levels){
                thisLevel.restoreState(oldState._levels.get(i++));
            }

            _timeSpentPaused = oldState._timeSpentPaused;
            _currentLevelStartTime = oldState._currentLevelStartTime;
            _infinitePlayerMissiles = oldState._infinitePlayerMissiles;
            _playerMissileSpeed = oldState._playerMissileSpeed;
            _enemyMissileSpeed = oldState._enemyMissileSpeed;
            _noLevels = oldState._noLevels;
            _currentLevelNumber = oldState._currentLevelNumber;
            _currentScore = oldState._currentScore;
            _stoppedMissileCount = oldState._stoppedMissileCount;
            _stateBeforePause = oldState._stateBeforePause;

            for(Object thisKey : oldState._stateStartTime.keySet())
                _stateStartTime.put((GameState)thisKey, oldState._stateStartTime.get(thisKey));

            _initialScudSpeed = oldState._initialScudSpeed;
            _scudsPerLevel = oldState._scudsPerLevel;

            // adjust for the time delay before restoring the state
            _timeSpentPaused += getAdjustedTimeInMillis() - oldState._lastTimeValue;
            _state = oldState._state;

            _oldState.cleanup();
            _oldState = oldState = null;
            _haveRestoreStateRequest = false;
            return;
        }
    ...
    private void buildLevels(){
        ...
        for(int i = 0; i < _noLevels; ++i){
            //                        noEnemyMissiles       enemyMissileSpeed        levelLength (seconds)   LevelNo
            _levels.add(new GameLevel( _scudsPerLevel + 2 * i, _initialScudSpeed + 5 * i,         25 ,               i));
        }
    }
    ...
    private void startNewLevel(boolean newGame){
        ...
        if(!newGame) {
            ...
            int standingSiloCount = 0;
            int standingCityCount = 0;
            // Remaining player missiles
            for (Silo thisSilo : _silos)
                standingSiloCount += (thisSilo.getState() == GroundAssetState.invisible) ? 0 : 1;
            // Remaining Cities
            for (City thisCity : _cities)
                standingCityCount += (thisCity.getState() == GroundAssetState.invisible) ? 0 : 1;


            _currentScore += _stoppedMissileCount * 20;
            _currentScore += standingCityCount * 100;
            _currentScore += standingSiloCount * 40;
    ...
    public void restoreState(GameFieldView oldState){
        _oldState = oldState;
        _haveRestoreStateRequest = true;
    }
    ...
    public void cleanup(){
        ...
        _stateStartTime.clear();
        _stateStartTime = null;

        _stateLength.clear();
        _stateLength = null;

        _stateTransitionMap.clear();
        _stateTransitionMap = null;
    }
    ...
    private long getAdjustedTimeInMillis(){
        return _lastTimeValue = (System.currentTimeMillis() - _timeSpentPaused);
    }

    /* --------------------------------------------------------------*
    *                         Status Display                         *
    * ---------------------------------------------------------------*/
    private class Status implements Renderable<Status>, Serializable{
        private transient Drawable _levelIndicator;
        private transient Drawable _scoreIndicator;
    ...
    * ---------------------------------------------------------------*/
    private class GameLevel implements Serializable
    {
        ...
        public GameLevel(int noEnemyMissiles, int enemyMissileSpeed, long levelLength, int levelNo)
        {
        ...
        public void restoreState(GameLevel otherLevel){
            if(otherLevel == null)
                throw new NullPointerException();

            _levelLength = otherLevel._levelLength;
            _enemyMissileSpeed = otherLevel._enemyMissileSpeed;
            _levelNo = otherLevel._levelNo;
            _noLaunchedEnemyMissiles = otherLevel._noLaunchedEnemyMissiles;
            _noEnemyMissiles = otherLevel._noEnemyMissiles;
            _missileSchedule = new long[_noEnemyMissiles];
            for(int i = 0; i < _missileSchedule.length; ++i)
                _missileSchedule[i] = otherLevel._missileSchedule[i];
        }
    ...
    * ----------------------------------------------------------------*/
    ...
    private abstract class GroundAsset implements Renderable<GroundAsset>, Serializable{
        protected Point _position;
        protected transient PositionedPath _path;
    ...
    * ---------------------------------------------------------------*/
    ...
    private class Missile implements Renderable<Missile>, Serializable{
        private Point _destination;
        private Point _currentLocation;
        private Point _launchpadLocation;
        private MissileState _state;
        private MissileType _type;
        private transient Paint   _tailColor;
        private transient Line    _tail;
        private transient Circle _explosion;
        private double _speed;
        private double _launchTime;
        private double _explodeStartTime;
        private float _maxBlastRadius;
        private float _currentBlastRadius;
        private double _explosionDuration;
        private double _launchAngle;
        private float _launchingHeadSize = 1;
        private double _expectedArrivalTime;
        private int _frameNo;
        private int _currentExplosionColor;
        private boolean _relaunchable;
        private transient PositionedPath _crosshair;
        private transient GameFieldView _caller;
        ...
        public void restoreState(Missile otherMissile){
            if(otherMissile == null)
                throw new NullPointerException();

            _state = otherMissile._state;
            _launchpadLocation = otherMissile._launchpadLocation.clone();
            _destination = otherMissile._destination.clone();
            _currentLocation = otherMissile._currentLocation.clone();
            _type = otherMissile.getType();
            _tailColor = PaintBuilder.buildPaint(Paint.Style.STROKE, (_type == MissileType.enemy) ? Color.RED : Color.BLUE, 1);
            _launchAngle = otherMissile._launchAngle;
            _launchTime = otherMissile._launchTime;
            _currentExplosionColor = otherMissile._currentExplosionColor;
            _explodeStartTime = otherMissile._explodeStartTime;
            _frameNo = otherMissile._frameNo;
            _speed = otherMissile._speed;
            _expectedArrivalTime = otherMissile._expectedArrivalTime;
            _currentBlastRadius = otherMissile._currentBlastRadius;
            _tail = new Line(_launchpadLocation, _currentLocation, _tailColor);
            _crosshair = new PositionedPath(buildPath(VisibleGameObject.crosshair),_destination,PaintBuilder.buildPaint(Paint.Style.STROKE, _currentExplosionColor,1));
            _explosion = new Circle(_currentLocation, PaintBuilder.buildPaint(Paint.Style.FILL, _currentExplosionColor, 1),_currentBlastRadius);
        }
        ...
        public boolean tryLaunch(Point launchpadLocation, Point destination, double speed)
        {
            ...
            if (_state == MissileState.ready)
            {
                ...
                _tail = new Line(_launchpadLocation, _launchpadLocation, _tailColor);
                _explosion = new Circle(_currentLocation, PaintBuilder.buildPaint(Paint.Style.FILL, _currentExplosionColor, 1),_currentBlastRadius);
                return true;
            }
            return false;
        ...
        public void timerCLick(double currentTimeInMillis) {
            ...
            if(_state ==  MissileState.launching) {
                _currentBlastRadius = _launchingHeadSize;
                ...
                _explosion.setRadius(_currentBlastRadius);
            ...
            if(_state == MissileState.exploding) {
                ...
                _currentBlastRadius = (float) Math.abs(_maxBlastRadius * Math.sin(Math.PI * (millisSinceExplosion / (_explosionDuration * 1000))));
                _explosion.setRadius(_currentBlastRadius);
                _explosion.setPosition(_currentLocation);
            }

            if(_state == MissileState.launching || _state == MissileState.exploding) {
                _currentExplosionColor = 0;
                int frameNoMod = _frameNo % 10;
                // white
                if (frameNoMod == 0 || frameNoMod == 4 || frameNoMod == 8)
                    _currentExplosionColor = Color.WHITE;
                // red
                else if (frameNoMod == 1 || frameNoMod == 5 || frameNoMod == 9)
                    _currentExplosionColor = Color.RED;
                // yellow
                else if (frameNoMod == 2 || frameNoMod == 6)
                    _currentExplosionColor = Color.YELLOW;
                // orange
                else if (frameNoMod == 3 || frameNoMod == 7)
                    _currentExplosionColor = Color.rgb(0xFF, 0xA5, 0x00);

                _explosion.getPaint().setColor(_currentExplosionColor);
                _crosshair.getPaint().setColor(_currentExplosionColor);
            }
        ...
        public Missile(MissileType type, boolean relaunchable, GameFieldView caller) {
            ...
            _maxBlastRadius = 12.5f;
            _currentBlastRadius = _launchingHeadSize;
            _explosionDuration = 2;
            _frameNo = 0;
            _relaunchable = relaunchable;
            _explosion = new Circle(_currentLocation, PaintBuilder.buildPaint(Paint.Style.FILL, Color.WHITE, 1),_currentBlastRadius);
            _crosshair = new PositionedPath(buildPath(VisibleGameObject.crosshair),_destination,PaintBuilder.buildPaint(Paint.Style.STROKE, Color.BLUE,1));
}
